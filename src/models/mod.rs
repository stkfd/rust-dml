...
pub trait UnSupModel<Inputs: IndexableData, Predictions, TrainingOutput> {
    ...
    fn train<S: Scope, Source: DataSourceSpec<Inputs>>(&mut self, scope: &mut S, inputs: Source) -> Result<Stream<S, TrainingOutput>>;
}

pub trait StreamingUnSupModel<Inputs: IndexableData, Predictions, TrainingOutput> {
    /// Predict output from inputs.
    fn predict<S: Scope>(&mut self, scope: &mut S, inputs: Stream<S, Inputs>) -> Result<Stream<S, Predictions>>;

    /// Train the model using inputs.
    fn train<S: Scope>(&mut self, scope: &mut S, inputs: Stream<S, Inputs>) -> Result<Stream<S, TrainingOutput>>;
}