...
{
    fn fetch_items(&self, provider_spec_stream: Stream<S, Ps>) -> Stream<S, Items> {
        self.binary_frontier(&provider_spec_stream, Pipeline, Pipeline, "fetch_items", |_, _| {
            ...
            move |in_items, in_provider, out| {
                ...
                in_items.for_each(|time, data| {
                    item_spec_stash.entry(time.retain())
                ...
                in_provider.for_each(|time, data| {
                    ...
                    match provider_spec_stash.entry(time.retain()) {
                ...
                for (time, slice_indices) in &mut item_spec_stash {
                    if !in_provider.frontier().less_equal(time) {
                ...
                provider_spec_stash.retain(|time, _| in_items.frontier().less_equal(time))
            }
        })
    }
}
